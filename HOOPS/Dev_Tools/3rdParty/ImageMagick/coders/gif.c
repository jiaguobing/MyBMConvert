...
#define NullCode  (~0UL)
  ...
  */
  ...
  suffix=(unsigned char *) AcquireMagickMemory(MaxStackSize*sizeof(*suffix));
  pixel_stack=(unsigned char *) AcquireMagickMemory((MaxStackSize+1)*
    sizeof(*pixel_stack));
  if ((packet == (unsigned char *) NULL) || (prefix == (short *) NULL) ||
        ...
        (image->matte != MagickFalse) && (image->colors <= 256))
      {
        ...
        */
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          {
            if ((double) p->opacity > ((QuantumRange+image->fuzz)/2.0))
              {
