        ...
        */
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (image->matte == MagickFalse)
            (void) ExportQuantumPixels(image,CMYKQuantum,0,pixels+offset);
          else
            (void) ExportQuantumPixels(image,CMYKAQuantum,0,pixels+offset);
      ...
      case LineInterlace:
      {
        ...
        */
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (q == (PixelPacket *) NULL)
            break;
          (void) ExportQuantumPixels(image,CyanQuantum,0,pixels+offset);
          count=ReadBlob(image,packet_size*image->extract_info.width,pixels);
          if (count != (ssize_t) (packet_size*image->extract_info.width))
            break;
          (void) ExportQuantumPixels(image,MagentaQuantum,0,pixels+offset);
          count=ReadBlob(image,packet_size*image->extract_info.width,pixels);
          if (count != (ssize_t) (packet_size*image->extract_info.width))
            break;
          (void) ExportQuantumPixels(image,YellowQuantum,0,pixels+offset);
          count=ReadBlob(image,packet_size*image->extract_info.width,pixels);
          if (count != (ssize_t) (packet_size*image->extract_info.width))
            break;
          (void) ExportQuantumPixels(image,BlackQuantum,0,pixels+offset);
          if (image->matte != MagickFalse)
            {
              count=ReadBlob(image,packet_size*image->extract_info.width,
                pixels);
              if (count != (ssize_t) (packet_size*image->extract_info.width))
                break;
              (void) ExportQuantumPixels(image,AlphaQuantum,0,pixels+offset);
            }
          if (SyncImagePixels(image) == MagickFalse)
            break;
          if (image->previous == (Image *) NULL)
