        ...
        */
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (image->matte == MagickFalse)
            (void) ExportQuantumPixels(image,RGBQuantum,0,pixels+offset);
          else
            if (LocaleCompare(image_info->magick,"RGBA") == 0)
              (void) ExportQuantumPixels(image,RGBAQuantum,0,pixels+offset);
            else
              (void) ExportQuantumPixels(image,RGBOQuantum,0,pixels+offset);
      ...
      case LineInterlace:
      {
        ...
        */
        ...
        for (y=0; y < (long) image->rows; y++)
        {
          ...
          if (q == (PixelPacket *) NULL)
            break;
          (void) ExportQuantumPixels(image,RedQuantum,0,pixels+offset);
          count=ReadBlob(image,packet_size*image->extract_info.width,pixels);
          if (count != (ssize_t) (packet_size*image->extract_info.width))
            break;
          (void) ExportQuantumPixels(image,GreenQuantum,0,pixels+offset);
          count=ReadBlob(image,packet_size*image->extract_info.width,pixels);
          if (count != (ssize_t) (packet_size*image->extract_info.width))
            break;
          (void) ExportQuantumPixels(image,BlueQuantum,0,pixels+offset);
          if (image->matte != MagickFalse)
            {
              count=ReadBlob(image,packet_size*image->extract_info.width,
                pixels);
