...
MagickExport Image *CoalesceImages(Image *image,ExceptionInfo *exception)
{
  ...
  */
  ...
  for ( ; next != (Image *) NULL; next=GetNextImageInList(next))
  {
    switch (next->previous->dispose)
    {
      ...
      case BackgroundDispose:
      {
        ...
        SetImageBackgroundColor(coalesce_image->next);
        break;
      }
      case PreviousDispose:
      default:
      {
        coalesce_image->next=CloneImage(dispose_image,0,0,MagickTrue,
          exception);
        break;
      }
    }
  ...
  if (dispose_image != (Image *) NULL)
    dispose_image=DestroyImage(dispose_image);
  next=GetFirstImageInList(coalesce_image);
  next->dispose=BackgroundDispose;
  for ( ; next->next != (Image *) NULL; next=GetNextImageInList(next))
  {
    next->dispose=BackgroundDispose;
    if (IsOpaqueImage(next->next,exception) == MagickFalse)
      next->dispose=NoneDispose;
    next->next->dispose=next->dispose;
  }
