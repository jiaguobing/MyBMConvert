  ...
  const PixelPacket *p,const PixelPacket *q)
{
  ...
  pixel=(MagickRealType) p->red-(MagickRealType) q->red;
  distance=pixel*pixel;
  if (distance > fuzz)
    return(MagickFalse);
  pixel=(MagickRealType) p->green-(MagickRealType) q->green;
  distance+=pixel*pixel;
  if (distance > fuzz)
    return(MagickFalse);
  pixel=(MagickRealType) p->blue-(MagickRealType) q->blue;
  distance+=pixel*pixel;
  if (distance > fuzz)
    return(MagickFalse);
  if (image->matte != MagickFalse)
  ...
  for (i=0; i < (long) number_colors; i++)
  {
    ...
    if (color_info[i]->color.opacity == OpaqueOpacity)
      ...
      (void) fprintf(file,"rgba(%3u,%3u,%3u,%3.1g)     ",
        ...
        ScaleQuantumToChar(color_info[i]->color.green),
        ScaleQuantumToChar(color_info[i]->color.blue),
        (double) (QuantumRange-color_info[i]->color.opacity)/QuantumRange);
          ...
          if (LocaleCompare(keyword,"file") == 0)
            {
              if (depth > 200)
              ...
              else
                {
                  ...
                  xml=FileToString(path,(const size_t)~0,exception);
