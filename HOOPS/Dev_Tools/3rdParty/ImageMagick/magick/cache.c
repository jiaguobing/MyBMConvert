...
static int CloseDiskCache(CacheInfo *cache_info)
{
  int
    status;
...
static void LimitOpenCacheDescriptors(void)
{
  ...
  */
  ...
  {
    if ((p->type == DiskCache) && (p->file != -1) &&
        (p->thread == GetCacheThreadId()) && (p->timestamp < q->timestamp))
      q=p;
    p=(CacheInfo *) GetNextKeyInSplayTree(cache_resources);
  }
  if (q != (CacheInfo *) NULL)
...
static int OpenDiskCache(CacheInfo *cache_info,MapMode mode)
{
  int
    file;
  ...
  */
  ...
  switch (mode)
  {
    ...
    default:
    {
      file=open(cache_info->cache_filename,O_RDWR | O_CREAT | O_BINARY | O_EXCL,
        S_MODE);
      if (file == -1)
  ...
  CacheInfo *source_info,ExceptionInfo *exception)
{
  ...
  if (cache_info->file == -1)
    {
      cache_info->file=OpenDiskCache(cache_info,IOMode);
      if (cache_info->file == -1)
        {
  ...
  if (source_info->file == -1)
    {
      source_info->file=OpenDiskCache(source_info,IOMode);
      if (source_info->file == -1)
        {
          ThrowFileException(exception,FileOpenError,"UnableToOpenFile",
  ...
  ExceptionInfo *exception)
{
  ...
  */
  ...
  if (*cache_info->cache_filename != '\0')
    file=OpenDiskCache(cache_info,mode);
  else
    {
      LimitOpenCacheDescriptors();
