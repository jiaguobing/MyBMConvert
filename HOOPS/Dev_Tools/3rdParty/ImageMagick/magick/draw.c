  ...
  const Image *source,const AffineMatrix *affine)
{
  ...
  */
  ...
  for (y=edge.y1; y <= edge.y2; y++)
  {
    inverse_edge=AffineEdge(source,&inverse_affine,y,&edge);
    if (inverse_edge.x2 < inverse_edge.x1)
      continue;
    q=GetImagePixels(image,(long) inverse_edge.x1,(long) y,(unsigned long)
      (inverse_edge.x2-inverse_edge.x1+0.5),1);
    if (q == (PixelPacket *) NULL)
      continue;
    indexes=GetIndexes(image);
    x_offset=0;
    for (x=inverse_edge.x1; x <= inverse_edge.x2; x++)
    {
      ...
      pixel=InterpolateColor(source,point.x,point.y,&image->exception);
...
#define RenderImageTag  "Render/Image"
  ...
  */
  ...
  if (*draw_info->primitive != '@')
    ...
    primitive=FileToString(draw_info->primitive+1,(const size_t)~0,&image->exception);
