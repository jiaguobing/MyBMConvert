...
void HUtilitySparseShell::AddOneIndexToPair (int bigIndex, int littleIndex)
{
	if (m_i2p) {
		...
		vertexref = static_cast<unsigned int>((si_index << VI_BITS) | (littleIndex - si->offset));
...
bool HUtilitySparseShell::GetPair (int bigIndex, int n, HC_KEY *key, int *littleIndex) const
{
	...
	if (key)
		*key = si->key;
	if (littleIndex)
		*littleIndex = static_cast<int>(si->offset + (vertexref & VI_MASK));
	return true;
}
...
void HUtilitySparseShell::AddOneTranslation (void *arg1, void *arg2, void *arg3)
{
	...
	im->AddOneIndexToPair (static_cast<int>(bigIndex), static_cast<int>(littleIndex));
}
	   ...
	   const int *face_list)
{
	...
	while (face < end) {
		...
		for (i = 0 ; i < length ; i++) {
			...
			newface[i] = static_cast<int>(index);
		}
		face += length;
		newface += length;
	}
