...
#   endif
    ...
    void Reallocate( )
    {
        ...
        if( m_can_reallocate ) {
            unsigned int *temp;
            m_allocated *= 2;
			BSTREAM_ALLOC_ARRAY(temp, m_allocated, unsigned int);
            if( temp == NULL ) {
				...
				BSTREAM_FREE_ARRAY(m_data, m_allocated / 2, unsigned int);
                m_data = temp;
            }
        }
        else {
  ...
  public:
    ...
    ~BVarStream( )
    {
        if( m_data != NULL ) {
            if( m_can_reallocate )
				BSTREAM_FREE_ARRAY(m_data, m_allocated, unsigned int);
            m_data = NULL;
        }
    };
...
#endif
{
  ...
  public:
    ...
    void SetCanReallocate(int val)       { m_can_reallocate = val; };
