...
class BBINFILETK_API TK_Polyhedron : public BBaseOpcodeHandler {
	public:
		...
		*/
		...
		enum Opt_Opcode {
			...
			/**/
			/* Note: "UNCOMPRESSED" versions added instead of "COMPRESSED" to preserve original compressed meaning of like opts. */
			OPT_ALL_VFCOLORS_UNCOMPRESSED     = 101, //!< set uncompressed colors on all vertices, to apply to drawing of interpolated faces
			OPT_VERTEX_FCOLORS_UNCOMPRESSED   = 102, //!< set uncompressed colors on some vertices, to apply to drawing of interpolated faces
			OPT_ALL_VECOLORS_UNCOMPRESSED     = 103, //!< set uncompressed colors on all vertices, to apply to drawing of interpolated edges
			OPT_VERTEX_ECOLORS_UNCOMPRESSED   = 104, //!< set uncompressed colors on some vertices, to apply to drawing of interpolated edges
			OPT_ALL_VMCOLORS_UNCOMPRESSED     = 105, //!< set uncompressed colors on all vertices, to apply to drawing of vertex_markers 
			OPT_VERTEX_MCOLORS_UNCOMPRESSED   = 106, //!< set uncompressed colors on some vertices, to apply to drawing of vertex_markers 
			OPT_ALL_VMRGBAS_UNCOMPRESSED      = 107, //!< set uncompressed rgba colors on all vertices, to apply to drawing of vertex_markers 
			OPT_VERTEX_MRGBAS_UNCOMPRESSED    = 108, //!< set uncompressed rgba colors on some vertices, to apply to drawing of vertex_markers 
			OPT_ALL_VFINDICES_UNCOMPRESSED    = 109, //!< set uncompressed colors-by-index on all vertices, to apply to drawing of interpolated faces
			OPT_VERTEX_FINDICES_UNCOMPRESSED  = 110, //!< set uncompressed colors-by-index on some vertices, to apply to drawing of interpolated faces
			OPT_ALL_VEINDICES_UNCOMPRESSED    = 111, //!< set uncompressed colors-by-index on all vertices, to apply to drawing of interpolated edges
			OPT_VERTEX_EINDICES_UNCOMPRESSED  = 112, //!< set uncompressed colors-by-index on some vertices, to apply to drawing of interpolated edges
			OPT_ALL_VMINDICES_UNCOMPRESSED    = 113, //!< set uncompressed colors-by-index on all vertices, to apply to drawing of vertex_markers 
			OPT_VERTEX_MINDICES_UNCOMPRESSED  = 114, //!< set uncompressed colors-by-index on some vertices, to apply to drawing of vertex_markers 
			OPT_ALL_FACE_COLORS_UNCOMPRESSED  = 115, //!< set uncompressed colors on all faces
			OPT_FACE_COLORS_UNCOMPRESSED      = 116, //!< set uncompressed colors on some faces
			OPT_ALL_FACE_INDICES_UNCOMPRESSED = 117, //!< set uncompressed colors-by-index on all faces
			OPT_FACE_INDICES_UNCOMPRESSED     = 118, //!< set uncompressed colors-by-index on some faces
		};
		...
		/*! \return a pointer to the array of vertex locations */
		double const *GetDPoints() const { return mp_dpoints; };
		/*! \return a pointer to the array of vertex locations */
		double *GetDPoints() { return mp_dpoints; };
		...
		int *       GetFaceRegions()       { return mp_fregions; };

		/*! Gets the priority to this object */
		int  GetPriority ()				{return mp_priority;}
		/*! Sets priority to this object */
		void SetPriority (int priority) { mp_priority = priority; }
		


		//!informs the TK_Polyhedron of the id by which this piece of geometry is known
		TK_Status SetKey (BStreamFileToolkit &tk, ID_Key key)       { set_last_key (tk, key); mp_key = key; return TK_Normal; };
		...
		TK_Status SetSubop (unsigned char subop)       { mp_subop = subop; return TK_Normal; };
		...
		TK_Status SetSubop2 (unsigned short subop2)       { mp_subop2 = subop2; return TK_Normal; };
		...
		TK_Status SetBounding (float const bbox[])       { mp_bbox[0] = bbox[0]; mp_bbox[1] = bbox[1]; mp_bbox[2] = bbox[2]; mp_bbox[3] = bbox[3]; mp_bbox[4] = bbox[4]; mp_bbox[5] = bbox[5]; return TK_Normal; };
		...
		bool HasOptionals() { return
			...
			HasFaceAttributes() ||
			HasEdgeAttributes() ||
			(!(mp_subop2 & 0x01) && mp_num_collection_parts > 0);// 0x01 matches TKSH2_COLLECTION
		};

		bool HasOptionalsCommunicator() { return
			HasVertexAttributes() ||
		...
		inline bool HasFaceAttributesCommunicator() const { return 
			HasFaceRegions();
		};
	...
	protected:
		...
		int				mp_priority;	 /*! Priority of this object */
		float *			mp_points;       /*!< Array of floats which denote the point coordinates. */
		double *		mp_dpoints;       /*!< Array of floats which denote the point coordinates. */
		...
		virtual		TK_Status	read_trivial_points (BStreamFileToolkit & tk); /*!< internal use only */
		virtual		TK_Status	write_trivial_points (BStreamFileToolkit & tk); /*!< internal use only */

		TK_Status	read_vertex_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_normals_compressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_normals_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_normals_compressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_parameters (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_parameters_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_colors_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_colors (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_colors_uncompressed_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_colors_uncompressed (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices_uncompressed_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices_uncompressed (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_marker_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_marker_symbols (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_marker_sizes (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors_uncompressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors_uncompressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices_uncompressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices_uncompressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_regions (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_patterns (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_colors (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_indices (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_patterns (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_weights (BStreamFileToolkit & tk); /*!< internal use only */
		virtual		TK_Status read_collection (BStreamFileToolkit & tk); /*!< internal use only */

		TK_Status	write_vertex_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_compressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_compressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_parameters (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_parameters_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_colors (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_colors_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_colors_uncompressed (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_colors_uncompressed_all (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_indices (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_indices_all (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_indices_uncompressed (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_indices_uncompressed_all (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_marker_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_marker_symbols (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_marker_sizes (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors_uncompressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors_uncompressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices_uncompressed (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices_uncompressed_all (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_regions (BStreamFileToolkit & tk);  /*!< internal use only */
		TK_Status	write_face_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_patterns (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_colors (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_indices (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_normals (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_visibilities (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_patterns (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_weights (BStreamFileToolkit & tk); /*!< internal use only */
		virtual		TK_Status write_collection (BStreamFileToolkit & tk, bool write_tag = false, int variant = 0); /*!< internal use only */
		...
		TK_Status	write_vertex_normals_main (BStreamFileToolkit & tk); 
		/*! internal use only --
		...
		TK_Status	write_vertex_parameters_main (BStreamFileToolkit & tk); 
		/*! internal use only --
		...
		TK_Status	write_vertex_colors_main (BStreamFileToolkit & tk, unsigned char base_op);
		/*! internal use only --
			...
			Dispatches to one of the helper functions.  \param tk the toolkit \param op determines which of
			[face, edge, marker] vertex colors we are currently writing \returns success or failure */
		TK_Status	write_vertex_indices_main (BStreamFileToolkit & tk, unsigned char base_op); 
		/*! internal use only --
		...
		TK_Status	write_face_colors_main (BStreamFileToolkit & tk); 
		/*! internal use only --
			write the face colors (by index) as currently described in mp_exists, mp_findices, mp_findexcount.  
			Called by TK_Polyhedron::write().  Dispatches to one of the helper functions.  
			\param tk the toolkit \returns success or failure */
		TK_Status	write_face_indices_main (BStreamFileToolkit & tk);


		TK_Status	ReadAscii (BStreamFileToolkit & tk); //!< Deprecated
		TK_Status	WriteAscii (BStreamFileToolkit & tk); //!< Deprecated

		virtual		TK_Status	read_trivial_points_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		virtual		TK_Status	write_trivial_points_ascii (BStreamFileToolkit & tk); /*!< internal use only */

		TK_Status	read_vertex_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */

		TK_Status	read_vertex_normals_compressed_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_normals_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_normals_compressed_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_parameters_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_parameters_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_colors_all_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_colors_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices_all_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_indices_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	read_vertex_marker_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_marker_symbols_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_vertex_marker_sizes_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_colors_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_indices_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_regions_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_face_patterns_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_colors_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_indices_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_patterns_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	read_edge_weights_ascii (BStreamFileToolkit & tk); /*!< internal use only */

		virtual		TK_Status read_collection_ascii (BStreamFileToolkit & tk); /*!< internal use only */

		TK_Status	write_vertex_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_compressed_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_normals_compressed_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_parameters_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_parameters_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_colors_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_colors_all_ascii (BStreamFileToolkit & tk, unsigned char op); /*!< internal use only */
		TK_Status	write_vertex_indices_ascii (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_indices_all_ascii (BStreamFileToolkit & tk, unsigned char op);  /*!< internal use only */
		TK_Status	write_vertex_marker_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_vertex_marker_symbols_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		virtual		TK_Status	write_vertex_marker_sizes_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_colors_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_indices_all_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_regions_ascii (BStreamFileToolkit & tk);  /*!< internal use only */
		TK_Status	write_face_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_face_patterns_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_colors_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_indices_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_normals_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_visibilities_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_patterns_ascii (BStreamFileToolkit & tk); /*!< internal use only */
		TK_Status	write_edge_weights_ascii (BStreamFileToolkit & tk); /*!< internal use only */

		virtual		TK_Status write_collection_ascii (BStreamFileToolkit & tk, bool write_tag = false, int variant = 0); /*!< internal use only */


		/*! internal use only --
			write the vertex normals as currently described in mp_normals, mp_exists, and mp_normalcount.  
			Dispatches to one of the helper functions.  \param tk the toolkit \returns success or failure */
		TK_Status	write_vertex_normals_main_ascii (BStreamFileToolkit & tk); 
		/*! internal use only --
			write the vertex parameters (a.k.a texture coordinates) as currently described in mp_params, 
			mp_exists, and mp_paramcount. Called by TK_Polyhedron::write().  Dispatches to one of the 
			helper functions.  \param tk the toolkit \returns success or failure */
		TK_Status	write_vertex_parameters_main_ascii (BStreamFileToolkit & tk); 
		/*! internal use only --
			write the vertex colors (by value) as currently described in mp_exists, mp_vfcolors, mp_vecolors,
			mp_vmcolors, mp_vecolorcount, mp_vmcolorcount and mp_vfcolorcount.  Called by TK_Polyhedron::write().
			Dispatches to one of the helper functions.  \param tk the toolkit \param op determines which of
			[face, edge, marker] vertex colors we are currently writing \returns success or failure */
		TK_Status	write_vertex_colors_main_ascii (BStreamFileToolkit & tk, unsigned char base_op);
		/*! internal use only --
			write the vertex colors (by index) as currently described in mp_exists, mp_vfindices, mp_veindices,
			mp_vmindices, mp_veindexcount, mp_vmindexcount and mp_vfindexcount.  Called by TK_Polyhedron::write().
			Dispatches to one of the helper functions.  \param tk the toolkit \param op determines which of
			[face, edge, marker] vertex colors we are currently writing \returns success or failure */
		TK_Status	write_vertex_indices_main_ascii (BStreamFileToolkit & tk, unsigned char base_op); 
		/*! internal use only --
			write the face colors (by value) as currently described in mp_exists, mp_fcolors, mp_fcolorcount.  
			Called by TK_Polyhedron::write().  Dispatches to one of the helper functions.  
			\param tk the toolkit \returns success or failure */
		TK_Status	write_face_colors_main_ascii (BStreamFileToolkit & tk); 
		/*! internal use only --
			write the face colors (by index) as currently described in mp_exists, mp_findices, mp_findexcount.  
			Called by TK_Polyhedron::write().  Dispatches to one of the helper functions.  
			\param tk the toolkit \returns success or failure */
		TK_Status	write_face_indices_main_ascii (BStreamFileToolkit & tk);


		virtual	TK_Status mangle_points (BStreamFileToolkit & tk, int pointmap_count, int pointmap[]); /*!< internal use only */
		virtual	TK_Status mangle_point_attributes (BStreamFileToolkit & tk, int pointmap_count, int pointmap[]); /*!< internal use only */
		virtual	TK_Status mangle_face_attributes (BStreamFileToolkit & tk, int facemap_count, int facemap[]); /*!< internal use only */
		...
		TK_Status repulse_compress_normals (BStreamFileToolkit & tk, int bits);
		/*! Looks up a vector to use as the normal.  The last entry in the table is
			...
			\param bits The number of bits to use for each sample.  Only 8 or 10 are currently supported.
			\return The result (success or failure) of the function call. */ 
		TK_Status repulse_decompress_normals (BStreamFileToolkit & tk, int bits);

		//!obsolete.  Provided for compatibility.
		void set_points (int count, float const points[] = 0)       { SetPoints (count, points); };
		//!obsolete.  Provided for compatibility.
		void set_normals (float const normals[] = 0)       { SetVertexNormals (normals); };
		//!obsolete.  Provided for compatibility.
		void set_parameters (float const params[] = 0)       { SetVertexParameters (params); };
		//!obsolete.  Provided for compatibility.
		void set_vfcolors (float const colors[] = 0)       { SetVertexFaceColors (colors); };
		//!obsolete.  Provided for compatibility.
		void set_vecolors (float const colors[] = 0)       { SetVertexEdgeColors (colors); };
		//!obsolete.  Provided for compatibility.
		void set_vmcolors (float const colors[] = 0)       { SetVertexMarkerColors (colors); };
		//!obsolete.  Provided for compatibility.
		void set_vfindices (float const indices[] = 0)       { SetVertexFaceIndices (indices); };
		//!obsolete.  Provided for compatibility.
		void set_veindices (float const indices[] = 0)       { SetVertexEdgeIndices (indices); };
		//!obsolete.  Provided for compatibility.
		void set_vmindices (float const indices[] = 0)       { SetVertexMarkerIndices (indices); };
		//!obsolete.  Provided for compatibility.
		void set_fcolors (float const colors[] = 0)       { SetFaceColors (colors); };
		//!obsolete.  Provided for compatibility.
		void set_findices (float const indices[] = 0)       { SetFaceIndices (indices); };
		//!obsolete.  Provided for compatibility.
		void set_exists (unsigned int const exists[] = 0);
		//!obsolete.  Provided for compatibility.
		void set_face_exists (unsigned int const exists[] = 0);
		//!obsolete.  Provided for compatibility.
		void set_facemap (int count, int const facemap[] = 0);
...
class BBINFILETK_API TK_Mesh : public TK_Polyhedron {
	...
	public:
		...
		void  SetColumns (const int columns)       { m_columns = columns; }
