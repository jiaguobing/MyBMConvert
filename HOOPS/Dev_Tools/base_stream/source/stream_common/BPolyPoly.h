...
class BBINFILETK_API TK_PolyPolypoint : public BBaseOpcodeHandler {
    protected:
        ...
        double *        m_dpoints;              //!<The points array.
        ...
        float           m_dbbox[6];             //!<A bounding box around the unrestricted dimensions.  If the data is 2d, only the first 4 values are used, and it becomes a bounding rectangle.
        ...
        float           m_dtemprange[2];        //!<1d bounds of the stragglers
    ...
    public:
		...
		TK_Status   Execute (BStreamFileToolkit & tk);    /* non-default for tag processing */
        void        Reset ();
        ...
        /*! Sets the double-precision point array.  Allocates a buffer which will hold 'point_count' double triplets, and if specified,
            copies the 'points' into the buffer 
            \param point_count the aggregate total number of points in this instance of the opcode.
            \param points if provided, the actual double-precision floating-point values of the vertices. 
             \return a status value to indicate success or failure. */
        TK_Status   SetDPoints (int point_count, double const points[] = 0);
        /*! Sets the lengths array.  Allocates a buffer which will hold 'primitive_count' integers, and if specified,
